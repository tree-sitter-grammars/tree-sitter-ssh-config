LANGUAGE_NAME := tree-sitter-ssh-config
HOMEPAGE_URL := https://github.com/tree-sitter-grammars/tree-sitter-ssh-config
VERSION := 0.5.0
...
install: all
	install -d '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/ssh-config '$(DESTDIR)$(INCLUDEDIR)'/tree_sitter '$(DESTDIR)$(PCLIBDIR)' '$(DESTDIR)$(LIBDIR)'
...
ifneq ($(wildcard queries/*.scm),)
	install -m644 queries/*.scm '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/ssh-config
endif
...
uninstall:
	...
	$(RM) -r '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/ssh-config
